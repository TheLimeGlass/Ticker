...
import me.limeglass.ticker.utils.EnumClassInfo;
import me.limeglass.ticker.utils.TypeClassInfo;
import me.limeglass.ticker.utils.annotations.*;
...
public class Register {
	...
	public Register() {
		try {
			...
			File file = (File) method.invoke(Ticker.getInstance());
			...
			for (Enumeration<JarEntry> jarEntry = jar.entries(); jarEntry.hasMoreElements();) {
				...
				if (name.startsWith(Ticker.getPackageName()) && name.endsWith(".class")) {
			...
			if (!clazz.isAnnotationPresent(Disabled.class)) {
				...
				if (syntax != null) {
					if (Effect.class.isAssignableFrom(clazz)) {
						...
						Ticker.debugMessage("&5Registered Effect " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
					} else if (Condition.class.isAssignableFrom(clazz)) {
						Skript.registerCondition(clazz, syntax);
						Ticker.debugMessage("&5Registered Condition " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						...
						try {
							...
							Ticker.debugMessage("&5Registered Expression " + type.toString() + " " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						} catch (IllegalAccessException | IllegalArgumentException | InstantiationException e) {
							Ticker.consoleMessage("&cFailed to register expression " + clazz.getCanonicalName());
							e.printStackTrace();
						}
					}
				}
	...
	public static void metrics(Metrics metrics) {
		...
		Ticker.debugMessage("Metrics registered!");
	}
}