package me.limeglass.ticker.utils;
...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Ticker.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Ticker.getSyntaxData().set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Ticker.getInstance().getConfig().getString("syntaxDebug"))) Ticker.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Ticker.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Ticker.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Syntax.save();
	}
}