...
public abstract class TickerCondition extends Condition implements DataChecker {
	...
	public String toString(Event event, boolean debug) {
		...
		if (event == null) {
			Ticker.debugMessage(getClass().getSimpleName() + " - " + modSyntax);
		} else {
			Arrays.asList(expressions.getExpressions()).stream().forEach(expression->values.add(expression.toString(event, debug)));
			Ticker.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
		}
		return Ticker.getNameplate() + getClass().getSimpleName() + "- Syntax: " + getSyntax();
	}
